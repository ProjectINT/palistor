...
**Revision 2026-08-25**: the five open questions are closed — persist defaults to `{ maxPages: 3 }` with `'window'` as the opt-in; the scroll-anchor API ships with that opt-in; `refetch()` on infinite is confirmed as pull-to-refresh; resolver self-slot reads warn once in dev; sliding-window head truncation stays deferred to Phase 3 with `pagination.maxPages` reserved (same name as the persist cap, disambiguated by scope). **Nothing blocks Phase 1.** See "Resolved in round 4" near the end.

**Progress 2026-08-26 — Phases 1, 2 & 3 IMPLEMENTED — the plan is complete** (all three page modes on root AND nested per-entity lists; aggregate dirty; `maxPages` head truncation; boundary reflow; list Suspense; README EN/RU + demo section). 100 store tests in `store/pagination/*.test.ts` + 3 React tracking tests in `react/pagination.react.test.tsx`; full suite 1282 green; `executeListResolve` and non-paginated lists untouched. See the three "Implementation status" sections under Phasing for the file maps and the deliberate deviations. The only items NOT built are the two the ledger had already parked: EntityRegistry refcount GC (bodies stay in the registry by design — nothing observable needs it) and `refetch({ pages: 'loaded' })` (the chain refetch, rejected on correctness grounds; the name stays reserved).
...
### Implementation status — Phase 1 ✅ (2026-08-26)

**Shipped (file map).**
- `store/pagination/types.ts` — `PageRequest`, `PagedResult`, `PageCacheEntry`, `QueryFamily` (+ `settled`), `PaginationState` (+ `listPath`, `warned`), `PaginationPersistBlob`.
- `store/pagination/paginationController.ts` — `createPaginationState` (validation; `infinite`/`cursor` throw "Phase 2"), `windowOrdinals`/`projectWindow`, `computeQueryKey` (self-path exclusion, `queryKey` escape hatch), `bootstrapDeps` (deps ∪ filter `serverPaths`), `ensureCurrentFamily` (the single key-decision point: bootstrap / no-op / evict + reset-to-`initialPage`, carrying the settled dep set into the new family), family LRU (`maxCachedQueries`), page LRU with window pinning, `resetPagination`, `rekeyPagination` (with promotion), `deleteIdEverywhere`, derived accounting (`displayTotal`, `pageCountOf`, `hasNextPageOf`).
- `store/pagination/paginationPersist.ts` — `serializePagination` / `seedFamilyFromWindow` (rules 1–6 in their paged form, see deviations).
- `store/resolvePipeline/createLiveValuesSnapshotProxy.ts` — copy-on-read proxy over the live tree (repeatable reads, contained writes/deletes, hidden `$filters`, self-slot warn-once).
- `store/resolvePipeline/executePagedListResolve.ts` — the paged executor: per-family/per-ordinal `inFlight` (filed before the pending notify, released by identity), `generation` + family + ordinal gates, eager dep snapshots + per-path drift + completion-key gate, deps folded into both `fam.dependencies` and `state.dependencies`, run-scoped cross-page dedup, reconcile recipe, empty page ≠ wipe, `loading`/`isFetching` from `inFlight` drain.
- `store/init/createResolveManager.ts` — config-driven dispatch, `triggerPagedFetch` / `refetchPaginated` / `_retriggerPaginatedList`, diverts in both postNotifyHook loops, `retriggerByPaths` and the filter gate (all after the cycle-guard bump; the executor's drift path is guarded too).
- `store/buildProxy/buildListProxy.ts` — getters `page/pageSize/pageCount/total/serverTotal/hasNextPage/hasPrevPage/isFetching/isInitialLoading`, methods `setPage/nextPage/prevPage/refetch/invalidate`, mutation inversion in `add/remove/setItems`, `PAGINATION_SPREAD_KEYS` union; `react/createTrackingProxy.ts` tracks the getters; `store/constants.ts` reserves the keys; `Palistor` constructor throws on a fieldMapping collision.
- `store/store/registerNodes.ts` (attachment, root only), `entityRegistry.ts` (nested warn-once), `resetPipeline.ts` (per-page rollback), `palistor.ts` (`rekey`/`delete`/`restoreLists` routed through the controller), `persistManager.ts` (`__pagination` blob bound to the list's array), `defineList.ts` (`pagination` passthrough), `index.ts` (types exported).

**Deviations from the text above (deliberate, all tested).**
- **Bootstrap race → rename, not discard.** A family is `settled: false` until its first successful completion; that completion folds the accessed paths and **renames the family in place** to the key over the refined deps + live values (the values the resolver actually read), so the rows land under the correct key without a second request. The completion-key gate (6c) applies to **settled** families only, where a mismatch is genuine drift. Dep widening on later runs also renames in place (`_retriggerPaginatedList` never needs a widening escape).
- **`pageCount` derives from `serverTotal`**, not the display total — otherwise `hasNextPage` (`page < base + pageCount − 1`) would not be immune to an optimistic add. `total` stays the display value.
- **`remove()` stales the later pages only for a SERVER row** (present in some `initialIds`); removing an un-flushed local add shifts no server offset.
- **Empty page entries are kept** after delete/remove (never dropped) — `projectWindow` therefore never meets a missing window ordinal on the paged path.
- **A queryKey change clears the window immediately** (`isInitialLoading` reads true until page 1 lands) — a foreign query's rows are never rendered under the new key. `keepPreviousData` stays the Phase 2 opt-in.
- **`invalidate()` of the current page / whole family re-issues the current page now** (forced, reconcile recipe); other pages refetch on visit. **`reload()` on a paginated list is `refetch()`.**
- **Persist, Phase 1 shape:** the blob carries the current page (ids + initialIds + fetchedCount), the pointer, `serverTotal`, the settled deps **and their values** (`queryKeyAtSave` when the escape hatch is configured), and bodies of locally removed server rows. Rules 1–6 hold in that form: key recomputed at seed, serve-only-under-matching-key with the `$context` barrier, hydration never evicts itself (tested), `v`+fingerprint discard, blob bound to its array (attached only when the array survived pick/omit, ignored on length disagreement), synthesized stale family + `idle` fallback. **Persist hydrate now passes `valuesCache` to `applyPatch`** (previously the cache lagged until the next write) — required for the seed-time key recompute.
- `maxCachedQueries > 1` already retains families (LRU, no `gcTime`), so flip-back-to-a-cached-filter is served from cache when configured; default stays 1.

**Deferred at the time to Phase 2/3** — everything but the last three landed in Phase 2 (below): `infinite`/`cursor` modes, `prefetch`, `setPageSize`, finite-`staleTime` background revalidation, `keepPreviousData`, `gcTime`, `PersistOptions.onError` + quota retry + `revalidateOnHydrate` + `pagination.persist` bound + `disable()` clearing families, `pendingAdds`/`discardPendingAdds`; **still open:** per-entity nested lists, aggregate cross-page dirty, demo/README wiring.

**Test-plan coverage:** items 1–24 and 26–29 map onto A1–F7 (`pagination.test.ts`) and the two React tests; 25 = F4; 30–52 are covered by the Phase 2 suite (see its test map).

**Phase 2 — Richer modes & cache lifecycle.**
`infinite` mode, **cursor-first** (`loadMore` targeting `max(loadedOrdinals)+1` with append-on-success, window-membership projection gate, pinned window ordinals, `isFetchingNextPage`/`loadedPages`, per-page dirty rollup, offset-continuation gate for offset mode; row-proxy identity is already stable across appends — verified: `ListState → listProxyCache → per-list entityProxyCache → EntityNode`, upsert merges in place — so `React.memo` rows bail out correctly); `cursor` mode (`cursors` map, `nextCursor` threading, free `prevPage`, truncating invalidation, hydrated-cursor fallback); **targeted root recompute for list notifications** (verified feasible: a root list sources to group path `""` and groupDeps edges cover cross-group readers of the slot; inherits targeted recompute's documented lazy-edge-discovery gap; flip `notifyListChanged`'s root branch and the executor's completion recompute together — without it every `loadMore` is a full-tree recompute); `prefetch(n)` (hover/next-page warmup without moving the pointer); `keepPreviousData`/`isPreviousData`; finite `staleTime` background revalidation (honoring the reconcile recipe); multi-family retention (`maxCachedQueries > 1` + `gcTime`) so flip-back-to-old-filter is also cache-served; `setPageSize`; the infinite persist blob with its six safety rules (default `{ maxPages: 3 }`, `'window'` opt-in); the scroll-anchor API (`setScrollAnchor(id)` / `scrollAnchor`) shipped alongside that opt-in; the dev warn-once on resolver self-slot reads.

### Implementation status — Phase 2 ✅ (2026-08-26)

**Shipped.** `infinite` and `cursor` modes (`createPaginationState` accepts all three); `loadMore` / `isFetchingNextPage` / `loadedPages` / `continuationLost`; the cursor chain (`QueryFamily.cursors`, `ctx.page.cursor`, free `prevPage`, truncating `invalidate`, hydrated-cursor rebuild); the `Σ fetchedCount` continuation with the completion-time offset gate (step 6d) and its corrected reissue; per-page `dirty` rollup for infinite; `setItems` refusal on infinite; `add` placement on the last loaded ordinal; `prefetch(n)`; `setPageSize(n)`; `keepPreviousData` / `isPreviousData`; finite-`staleTime` stale-while-revalidate; multi-family retention (`maxCachedQueries > 1`) with `gcTime`; targeted root recompute for paginated list notifications (proxy `notifyListChanged`, the manager's navigation notify and the executor's completion notify all recompute scoped); the v2 persist blob (whole window, cursors, `pendingAdds`, scroll anchor) with `persist: false | 'window' | { maxPages }` (default `{ maxPages: 3 }`), `PersistOptions.onError`, the quota-retry ladder, `revalidateOnHydrate`, `pendingAdds` + `discardPendingAdds()`, `isStaleFromStorage`, `setScrollAnchor`/`scrollAnchor`, and family clearing on the account-switch path.

**Files:** `store/pagination/{types,paginationController,paginationPersist}.ts`, `store/resolvePipeline/executePagedListResolve.ts`, `store/init/createResolveManager.ts` (`loadMorePaginated` / `prefetchPaginated` / `invalidatePaginated` / `setPageSizePaginated` / `revalidatePaginated` / `_rebuildContinuation` / `_notifyPaginated`), `store/buildProxy/buildListProxy.ts`, `store/constants.ts` (`INFINITE_SPREAD_KEYS`, `CHAIN_SPREAD_KEYS`), `store/persist/{types,persistManager}.ts`, `react/createTrackingProxy.ts` (tracked-key set), `store/store/types.ts`.

**Deviations from the text above (deliberate, all tested).**
- **`cursors` live on `QueryFamily`, not `PaginationState`** — the chain must die with its family and survive an LRU page eviction (`entry.nextCursor` alone would not). `cursorFor` reads the map first, the predecessor's `nextCursor` second.
- **`loadedOrdinals` starts EMPTY in infinite mode** (an ordinal joins on success only), so `projectWindow` never meets a missing pinned ordinal before the first fetch.
- **Cursors are recorded only when the resolver actually threads them.** An offset feed must never look like a chain — otherwise every ordinal reads "sequential-only" and a plain `prefetch` is refused.
- **`prefetch` reachability is mode-aware:** a chain needs the predecessor's cursor; **infinite + offset only reaches `max(loadedOrdinals) + 1`**, because the continuation offset of an ordinal in between is unknown and issuing `(n − base) × pageSize` would file the SAME rows under two ordinals (cross-page dedup then empties the page that arrives second). Unreachable prefetches warn once and no-op.
- **A completion beyond `max + 1` stays cache-only** (no hole in the window) and is claimed by the next `loadMore` without a fetch; window membership is evaluated after the append, so an in-window background revalidation and an out-of-order completion both project.
- **`invalidate(k)` on infinite truncates and does NOT auto-refetch** — the truncated window is on screen and a background refetch would reflow under the user's scroll. Cursor mode re-issues `k` when it is the current page. `refetch()` stays exactly one request in every mode.
- **Stale-while-revalidate is scoped to TIME expiry.** A page past `staleTime` is projected and revalidated behind; a page marked `status: 'stale'` (an offset shift or an explicit `invalidate`) still blocks on its fetch, exactly as in Phase 1 — its rows are known wrong, not merely old.
- **`keepPreviousData` also suppresses `isInitialLoading`** (never a skeleton over rows that are on screen).
- **Persist blob is `v: 2`** (multi-page + cursors + `pendingAdds` + `anchorId`); a v1 blob is discarded by rule 4 and the restored array bootstraps through rule 6. **Rule 5a became a SUBSET check**: a bounded blob legitimately covers only a suffix of the value array. Trimmed head ordinals persist as `fetchedCount` **tombstones**, which is what keeps the continuation offset honest after a bounded restore.
- **`persist: false` writes no blob at all** (the value array still persists and bootstraps through rule 6) rather than a pointer-only blob.
- **Quota retry ladder:** full → pagination trimmed to the pointer → no pagination blob → `onError('save')`. Only a quota error retries smaller; a serializer throw or a dead driver fails once. Autosave is additionally suppressed while any paginated family has a fetch in flight (a torn window must not hydrate as `resolved`).
- **Families are cleared on `disable()`** — which a superseded `enable()` runs first, so that is the account-switch path — and the resolve state returns to `idle`, so the emptied list refetches under the live key instead of rendering empty forever. A FIRST `enable()` never clears (it would wipe an already-loaded window and, with `status: 'resolved'`, never refetch).
- **Key sets stay minimal per mode:** `continuationLost` ships for both chain modes (`CHAIN_SPREAD_KEYS`), `loadMore` / `isFetchingNextPage` / `loadedPages` for infinite only (`INFINITE_SPREAD_KEYS`). A paged list's key set is byte-for-byte the Phase 1 one.
- **Targeted root recompute is gated on `listState.pagination`.** Non-paginated root lists keep the full recompute: the stated goal was pagination's cost, and flipping every root list would take the lazy-edge-discovery gap along for lists that never asked for it.
- **`revalidateOnHydrate` fires only when the restored window is actually stale** (with `staleTime: Infinity` a hydrate is still exactly zero fetches), and `gcTime` timers are `unref`'d so a cache eviction never holds a process open.

**Test map.** Plan items 30–36, 38–42 → G1–G12; 32 → H3; 44, 48–52 → J1–J7; 43 → G12; cursor threading / free `prevPage` / sequential-only random access → H1–H4; `prefetch` / `setPageSize` (paged + infinite) / `keepPreviousData` / SWR / retention+`gcTime` → I1–I5; `persist: false` and `revalidateOnHydrate: 'all' | 'none'` → J8–J9; the infinite React surface (footer tracking + row-proxy identity across appends) → `react/pagination.react.test.tsx`. Items 37, 45–47 were already covered by the Phase 1 suite; 25 = F4.

**Still deferred to Phase 3:** per-entity (nested) paginated lists, aggregate cross-page `dirty` for paged, sliding-window head truncation (`pagination.maxPages`), display-level boundary reflow for paged deletes, EntityRegistry refcount GC on eviction, suspense integration, `refetch({ pages: 'loaded' })`, and README / demo wiring.

**Phase 3 — Per-entity lists & completeness.**
Refactor `triggerEntityListResolve` (`createResolveManager.ts` ~L324, which dedups on `status === 'pending'||'resolved'` with no deps-driven re-resolve) to route page fetches through the shared paged executor (per-page dedup, queryKey retrigger), giving nested paginated lists full parity; aggregate cross-page `dirty` for paged (per-page baselines + rollup); optional persistence of the whole page cache / pointer; optional EntityRegistry refcount GC on family/page eviction; **sliding-window head truncation for very long infinite sessions** (`pagination.maxPages`, default `Infinity` — the only sanctioned way a window ordinal leaves memory; truncated pages must leave a tombstone `fetchedCount` so the continuation offset survives, and `refetch()` truncates to `min(loadedOrdinals)` rather than `initialPage` — see round-4 decision 1); **display-level boundary reflow for paged deletes** (projection borrows the next page's head without writing any entry — the write-through variant was evaluated and rejected: borrowed rows land in `ids` but not `initialIds`, turning every downstream page permanently guard-dirty); suspense integration.

### Implementation status — Phase 3 ✅ (2026-08-26)

**Shipped.**
- **Nested (per-entity) paginated lists** through the shared paged executor. `EntityRegistry.getOrCreateEntityListState` attaches a `PaginationState` per `(owner, listConfigNode)` instance (the template-level placeholder never carries one; the old "not supported" warning is gone). `executePagedListResolve` is parameterized by three new deps — `pagedStateOf` (root: `resolveStates`; nested: `entityStates` keyed `(ownerId, listConfigNode)`), `pagedLiveValues` (root: the live tree; nested: the owner's flat snapshot — the legacy nested resolver contract, so `ctx.page` is simply added as arg 3) and owner-ref ingestion (`setEntityOwner`, upfront tmp ids). `triggerEntityListResolve` diverts a paginated instance to the executor (plain trigger = fetch the current page / serve a fully-fresh window without a request; `force` = `refetch()`). QueryKey retrigger: owner fields the resolver read arrive as `_entity_.<id>.<field>` paths and are matched per owner in `_retriggerEntityFieldResolves` (same `MAX_AUTO_RETRIGGERS` cap); `$context.*` deps are matched in `retriggerByPaths` over every nested instance (`hasNestedPaginated` gates both scans). `rekey` / `delete` / `reset` / persist `disable()` / `restoreLists` all reach nested instances (`forEachEntityList`); `reset()` PRESERVES the per-owner resolve state of a paginated nested list across `entityStates.clearAll()` (its cache survives the reset, so its status / settled dep set / cycle counter must too). `buildListProxy`'s owner branches of `add` / `setItems` route into the shared `pagedAdd` / `pagedSetItems`.
- **Aggregate cross-page `dirty` in every mode** — `isPaginatedDirty` is the per-page rollup over every cached page of the current family (paged/cursor used to be a window compare that read clean the moment the user navigated away from the edited page).
- **`pagination.maxPages`** (infinite): `truncateHead` runs after every successful append; dropped ordinals leave `loadedOrdinals` and stay in `fam.pages` as `fetchedCount` tombstones; un-flushed local rows of a dropped page are harvested onto the new head. `continuationOffset` / `fetchedSoFar` count loaded ordinals ∪ tombstones (never a cache-only prefetch); `evictPagesBeyond` neither counts nor evicts a tombstone; a tombstone is never re-fetched in place (`isTruncatedOrdinal` guards the executor, `prefetch`, `revalidate`); the executor's append gate refuses an ordinal below the window. `persist.maxPages` is clamped to `maxPages`; the blob carries the tombstones below the window, and a hydrated tombstone is NOT a window ordinal (`revalidateOnHydrate` targets the first live one; an all-tombstone blob is untrusted).
- **Display-level boundary reflow** (paged): `projectWindow` borrows the deficit of a short current page from a **stale** successor's head (the row that shifted into the page server-side) without writing any entry; once the successor is refetched the borrow ends and `staleShortPredecessor` marks the short page (`fetchedCount < pageSize` with a successor) stale so its next visit heals the one-row gap.
- **`options.suspense` for lists** — `items` / `length` / `map` / `values` throw the in-flight promise: a plain list (root or nested; the legacy nested path now keeps `state.promise`) while pending, a paginated list only while it has nothing renderable (`itemIds` empty and not `isPreviousData`), so navigation / `loadMore` / revalidation never suspend the rows on screen.
- **Rule-6 draft survival**: a synthesized family (restored array, no trusted blob — the only path a nested instance ever takes) keeps `_tmp_*` ids OUT of `initialIds` (a tmp id is by construction never a server row), so the first reconciling fetch re-appends the draft instead of silently replacing it.
- README (EN + RU) "Pagination" subsection + features row; demo: `orders` paginated list in `app-demo` (`OrdersPagedSection.tsx`, `fetchOrders`, i18n keys).

**Deviations from the text above (deliberate, all tested).**
- **`refetch()` on a head-truncated infinite window restarts from `initialPage`, not `min(loadedOrdinals)`.** Round-4 decision 1 feared `initialPage` "may itself have been dropped"; `truncateChain` handles that — the tombstone survivor is turned back into a plain stale entry, `loadedOrdinals` restarts from it and the forced fetch replaces it — so pull-to-refresh keeps decision 4's uniform "back to the top, exactly one request" semantics and clears the tombstones. Truncating to `min(loadedOrdinals)` would have left the user mid-feed with a stale prefix of tombstones.
- **The borrow reads the successor's `ids` and stops when the successor is fresh** (not "whenever the current page is short"): after a refetch the successor's head is no longer the shifted row, and borrowing it would render one row on two consecutive pages. The interim one-row gap is closed by staling the short page (server-truth deficit only — a local `remove()` keeps `fetchedCount`, so an un-flushed removal shows 19 rows until it is flushed and the page revisited, which matches D4's "offsets shifted" assumption).
- **Nested windows are not persisted as blobs** — their rows live inside the owner's array in the root list's array, and `restoreLists` seeds every nested instance through rule 6 (synthesized stale family + reconciling first fetch). The `_tmp_` baseline exclusion is what makes a restored nested draft survive.
- **Nested `loading` is `isFetching`** (the nodeState `loading` flag is a root-only concern — the list node is shared across owners).
- **Nested list notifications target the owner entity**, never the shared `listConfigNode` (root keeps the listConfigNode backward-compat bridge).
- **`triggerEntityListResolve` on a paginated instance is cache-aware**: a fully-fresh window is served without a request even when the per-owner resolve state was dropped.

**Not built, on purpose.**
- **EntityRegistry refcount GC on family/page eviction** — the ledger's "only if real" condition never materialized: entity bodies are shared across lists and templates, and an eviction-driven GC would need a global refcount over every list membership, projection object and template binding to be safe. `store.delete(id)` remains the eviction path; a day-long infinite session is bounded by `maxPages` now.
- **`refetch({ pages: 'loaded' })`** — the chain refetch was rejected on correctness grounds in round 3 (no atomicity, mutually dependent offsets, cursor re-entrancy); the name stays reserved and self-documenting for a later opt-in with its own design pass.

**Test map.** `store/pagination/pagination.phase3.test.ts`: K1–K8 nested paginated lists (independent windows / mutation inversion + owner refs + rekey + delete / owner-field queryKey / `$context` / reset + reload / infinite / persist bootstrap / spread keys); L1–L2 aggregate dirty (paged + cursor); M1–M7 `maxPages` (truncation + continuation, harvested drafts, refetch restart, persist tombstones + clamp, LRU/prefetch/revalidate immunity, validation, paged/cursor no-op); N1–N3 boundary reflow; O1–O3 suspense (paginated / plain root / nested). `D1` was updated to the aggregate-dirty semantics and `F4` to the nested-pagination parity. Plan test items 25 (now parity) and 13 (aggregate) changed meaning accordingly; everything else in 1–52 is unchanged.
...
## Resolved in round 4 (2026-08-25) — the open-questions list is closed

All five open questions were worked through and decided; nothing blocks Phase 1. Recorded here with the reasoning that survived, since each is a constraint on later phases.

1. **Sliding-window head truncation stays in Phase 3.** `pagination.maxPages` defaults to `Infinity` — the window is never truncated in Phases 1-2. The name and the constraints are reserved, not implemented: truncated pages must leave a **tombstone `fetchedCount`** (not `|initialIds|` — the continuation counter is `Σ fetchedCount`, see round 3) or the continuation offset breaks; `reset`'s rolled-back window would no longer equal the full history; window-ordinal LRU pinning makes head truncation the *only* sanctioned way a window ordinal leaves memory; and `refetch()`'s "truncate to `initialPage`" becomes "truncate to `min(loadedOrdinals)`", since `initialPage` may itself have been dropped. Day-long infinite sessions growing monotonically is accepted until then.

2. **`pagination.persist` defaults to `{ maxPages: 3 }`**, with `'window'` as the documented opt-in. A bounded default restores a usable tail *plus* the correct dirty/reset baseline while avoiding the three costs of full-window-by-default: per-save cost is O(window) on every global notify (a 40-page window clones ~800 POJOs and stringifies ~200 KB per keystroke in an unrelated field); the first over-quota `setItem` silently kills persistence for the entire form, at a scroll depth that never reproduces in dev; and the stated benefit is not actually delivered, because hydration lands after first paint, so the browser has already clamped scroll restoration to 0 against an empty document and the user scrolls back manually regardless. Full-window persistence remains *supported* — it is the default that is bounded.

3. **The scroll-anchor API ships with the `'window'` opt-in, not on the default path.** `list.setScrollAnchor(id)` (or an IntersectionObserver) records `anchorId` = first visible row into the persist blob; `list.scrollAnchor` is exposed after hydrate. It is written only when the author set it, so the default `{ maxPages: 3 }` blob pays nothing for it. Rationale for coupling: without an anchor, full-window persist pays every cost and still lands the user at the top — the opt-in would be half-delivered.

4. **`refetch()` on infinite drops to the first page — confirmed as specified.** Pull-to-refresh semantics: truncate to `initialPage`, exactly one request, uniform across all three modes. It discards scroll depth, and that is the accepted trade: the window-preserving alternative is exactly the chain refetch this ledger rejected on correctness grounds. Local un-flushed rows are harvested off the dropped pages onto the survivor, so the gesture never destroys user input. `refetch({ pages: 'loaded' })` stays a reserved, self-documenting name for apps that later accept the cost.

5. **Self-slot reads warn once in dev.** When the live snapshot proxy sees the resolver read its own list's materialized slot, warn once pointing at `ctx.page`. Such a read deep-copies the whole window and is excluded from both the queryKey and the drift set — silent cost with no invalidation benefit. Warn, not throw: a hard error would break legitimate resolvers that read their own window.

6. **`maxPages` is one concept on two layers** (raised by decision 2, which introduced the name into the persist block). `pagination.persist.maxPages` caps pages written to storage; `pagination.maxPages` (Phase 3) caps pages held in the window. Same react-query-familiar name, disambiguated by scope; the persist cap is clamped to the memory cap, since storing more pages than the window can hold is unreachable.
