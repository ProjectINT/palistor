...
describe("applyPatch", () => {
  ...
  it("does not update when the value is unchanged", () => {
  ...
  it("recursively walks nested groups", () => {
    ...
    expect(nodeState.get(zipNode)!.value).toBe(""); // untouched
    expect(changed.has(cityNode)).toBe(true);
    expect(changed.has(zipNode)).toBe(false);
  });

  it("updates several fields in one call", () => {
  ...
  it("skips CONFIG_PROPS keys (value, label, validate…)", () => {
    const leaf: AnyConfigNode = { value: "x" };
    const root: AnyConfigNode = { field: leaf };
    const nodeState = makeNodeState([[leaf, makeState("x")]]);

    // "value" is a CONFIG_PROP and must be skipped as a patch key
  ...
  it("adds into an existing changed Set", () => {
    ...
    // Must contain both leaf1 (from existing) and leaf2 (from the patch)
